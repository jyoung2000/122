...
def _ass_rounded_rect_abs(x: int, y: int, w: int, h: int, r: int) -> str:
    """Generate ASS drawing commands for a rounded rectangle at absolute coords.

    Used for \\clip(1, drawing) tags where coordinates are in PlayRes units.
    """
    r = min(r, min(w, h) // 2)
    if r <= 0:
    ...
    return (
        f"m {x + r} {y} "
        f"l {x + w - r} {y} "
        f"b {x + w - r + kr} {y} {x + w} {y + r - kr} {x + w} {y + r} "
        f"l {x + w} {y + h - r} "
        f"b {x + w} {y + h - r + kr} {x + w - r + kr} {y + h} {x + w - r} {y + h} "
        f"l {x + r} {y + h} "
        f"b {x + r - kr} {y + h} {x} {y + h - r + kr} {x} {y + h - r} "
        f"l {x} {y + r} "
        f"b {x} {y + r - kr} {x + r - kr} {y} {x + r} {y}"
    )
...
def generate_ass(
    ...
    # --- Apply rounded-corner \clip to subtitle background box ---
    # When _bg_split is active, add \clip(1, rounded_rect) to each base text
    # event (and _OL overlay event) to clip the BorderStyle=3 box to a rounded
    # rectangle shape.  If libass supports \clip with drawing commands, the box
    # gets rounded corners.  If not, the box renders with sharp corners as a
    # graceful fallback (BorderStyle=3 still works without the clip).
    if _bg_split and _bg_font_path:
        ...
        for ev_start, ev_end, ev_style, ev_text in base_text_events:
            ...
            if not plain_text.strip():
                new_base_events.append((ev_start, ev_end, ev_style, ev_text))
                continue
            ...
            # The box (BorderStyle=3) extends ol_width pixels beyond the text.
            # The clip area should match the box area with rounded corners.
            # Add a small extra margin (2px) to avoid clipping text at corners.
            clip_pad = ol_width + 2
            clip_w = round(rect_w + 2 * clip_pad)
            clip_h = round(total_h + 2 * clip_pad)
            clip_x = round((video_width - clip_w) / 2)
            # alignment=2 (bottom-center): text bottom at video_height - margin_v
            text_top_y = video_height - margin_v - total_h
            clip_y = round(text_top_y - clip_pad)

            clip_drawing = _ass_rounded_rect_abs(clip_x, clip_y, clip_w, clip_h, _bg_draw_radius)
            clip_tag = f"\\clip(1,{clip_drawing})"

            # Prepend \clip to the event text
            if ev_text.startswith("{"):
                # Insert clip tag into existing override block
                clipped_text = "{" + clip_tag + ev_text[1:]
            else:
                clipped_text = "{" + clip_tag + "}" + ev_text

            new_base_events.append((ev_start, ev_end, ev_style, clipped_text))
            _bg_clip_count += 1

        base_text_events[:] = new_base_events

        logger.info(
            "BG_CLIP: applied \\clip to %d base text events "
            "(radius=%dpx, font=%s)",
            _bg_clip_count, _bg_draw_radius, _bg_font_path,
        )
    ...
    #   Layer 0: base text events (border layer, uniform color; may have \clip for rounded bg)
    #   Layer 1: outline overlay events (when bg + outline both enabled)
    #   Layer N: drawing events (when _aw_bg_split, rounded-rect word backgrounds)
    ...
    logger.info(
        ...
        "bg=%s bg_radius=%d bg_clip=%s outline=%dpx speakers=%d segments=%d "
